  ...
  context "if user is admin" do
    ...
    describe "POST #create" do
      it "should create a image" do
        post :create,params:{product_id:@product.id,image: {width: 20,height:10, file:Rack::Test::UploadedFile.new(Rails.root.join('spec','support','images', 'top_logo.png'), 'image/png')}}
    ...
    describe "PUT #update" do
      it "should update a image" do
        put :update,params:{id: @image.id,product_id:@product.id,image: {width: 5,height:5, file:Rack::Test::UploadedFile.new(Rails.root.join('spec','support','images', 'top_logo.png'), 'image/png')}}
        expect(response).to redirect_to(admin_product_images_path(@product))
      end
    end
