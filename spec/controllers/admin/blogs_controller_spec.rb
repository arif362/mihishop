  ...
  context "if user is admin" do
    ...
    describe "POST #create" do
      it "should create a blog" do
        post :create,params:{blog: { title: " title",body:"this", cover_photo:Rack::Test::UploadedFile.new(Rails.root.join('spec','support','images', 'top_logo.png'), 'image/png')}}
        expect(response).to redirect_to(admin_blogs_path)
      end
      it "should render new" do
        post :create,params:{blog: { title: " title", cover_photo:Rack::Test::UploadedFile.new(Rails.root.join('spec','support','images', 'top_logo.png'), 'image/png')}}
        expect(response).to render_template("new")
      end
    end
    describe "PUT #update" do
      it "should update a blog" do
        blog = create(:blog)
        put :update,params:{id: blog.id,blog: { title: " title",body:"this", cover_photo:Rack::Test::UploadedFile.new(Rails.root.join('spec','support','images', 'top_logo.png'), 'image/png')}}
        expect(response).to redirect_to(admin_blogs_path)
      end
    end
